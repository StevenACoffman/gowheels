...
func (s *BuildSource) Resolve(ctx context.Context, name string, plats []platforms.Platform) ([]Binary, error) {
	...
	for _, p := range plats {
		...
		outPath := filepath.Join(tmpDir, fmt.Sprintf("%s_%s_%s%s", name, p.GOOS, p.GOARCH, p.BinaryExt()))
		fmt.Fprintf(s.stdout, "  building %s/%s...\n", p.GOOS, p.GOARCH)

		if err := compileGo(ctx, s.modDir, outPath, p.GOOS, p.GOARCH, s.pkg, s.ldflags, s.stderr); err != nil {
			return nil, err
		}
...
func compileGo(ctx context.Context, modDir, output, goos, goarch, pkg, ldflags string, stderr io.Writer) error {
	cmd := exec.CommandContext(ctx, "go", "build", "-ldflags="+ldflags, "-o", output, pkg) //nolint:gosec // intentional
	cmd.Dir = modDir
